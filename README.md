<div align="center">

# 📢 Ads-Manager

### Платформа для планирования рекламных кампаний


</div>

***

## 📋 О проекте

**Ads-Manager** — это платформа для удобного планирования, управления и отслеживания рекламных кампаний. Проект предоставляет инструменты для организации рекламного процесса от идеи до запуска.

***

## 🗂️ Структура проекта

```
Ads-Manager/
├── .github/          # Конфигурация GitHub Actions и шаблоны
├── src/              # Исходный код приложения
├── web/              # Веб-интерфейс
├── CONTRIBUTING.md   # Руководство для контрибьюторов
└── README.md         # Документация проекта
```

***

## 🚀 Быстрый старт

### Требования

- Git
- Node.js / Python *(в зависимости от стека)*
- Актуальная версия браузера

### Установка

```bash
# 1. Клонируйте репозиторий
git clone https://github.com/TessEndGrad/Ads-Manager.git

# 2. Перейдите в директорию проекта
cd Ads-Manager

# 3. Установите зависимости
npm install   # или pip install -r requirements.txt

# 4. Запустите проект
npm start     # или python main.py
```

***

## ✨ Возможности

- 📅 **Планирование кампаний** — создавайте и управляйте рекламными кампаниями
- 📊 **Аналитика** — отслеживайте эффективность размещений
- 🗃️ **Организация** — структурируйте кампании по категориям и меткам
- 🌐 **Веб-интерфейс** — удобная работа через браузер

***

## 🤝 Вклад в проект

Мы рады любому вкладу! Пожалуйста, ознакомьтесь с [руководством для контрибьюторов](CONTRIBUTING.md) перед тем, как создавать pull request.

1. Сделайте форк репозитория
2. Создайте ветку для вашей фичи (`git checkout -b feature/my-feature`)
3. Зафиксируйте изменения (`git commit -m 'feat: добавить новую функцию'`)
4. Запушьте в ветку (`git push origin feature/my-feature`)
5. Откройте Pull Request

***

## 📄 Лицензия

Этот проект распространяется под лицензией MIT. Подробнее см. в файле [LICENSE](LICENSE).

***

## 🪛 Известные баги

1. Посты появляются только после обновления страницы
2. На вкладке "Все публикации" при выборе фильтров не понятно какой из них выбран (не отображаются)
3. Не работает редактирование постов

***

<div align="center">

Сделано с ❤️ командой **TessEndGrad**

</div>
